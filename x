...
if Forbidden.Sentinel.CleanConsole.Enabled and Forbidden.Sentinel.CleanConsole.Mode == "auto" then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if Forbidden.Sentinel.CleanConsole.Mode == "keybind" and input.KeyCode == Enum.KeyCode[string.upper(Forbidden.Sentinel.KeyTrigger.ClearConsole)] then
        shouldClear = true
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode[string.upper(Forbidden.Sentinel.KeyTrigger.ClearConsole)] then
...
if Forbidden.Sentinel.Memory.IsActive then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = Forbidden.Sentinel.Memory.Params.MinValue
            local maxMemory = Forbidden.Sentinel.Memory.Params.MaxValue
...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = Forbidden.Sentinel.BoxFov.Height * 100
    local fovWidth = Forbidden.Sentinel.BoxFov.Width * 100
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[Forbidden.Sentinel.KeyTrigger.TriggerBot:upper()] then
        isToggled = true
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[Forbidden.Sentinel.KeyTrigger.Visuals:upper()] then
...
local function updateTracer()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if Forbidden.Sentinel.Visuals.TargetClosestBodyPart then
        ...
        if targetPart then
            if not TracerLine then
                ...
                TracerLine.Thickness = Forbidden.Sentinel.Visuals.TracerThickness
                TracerLine.Transparency = Forbidden.Sentinel.Visuals.TracerTransparency
            end
...
local function handleESP()
    if visualsActive and Forbidden.Sentinel.Visuals.Enabled then
...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = Forbidden.Sentinel.CamLock.Normal.Radius_Visibility
            Circle.Radius = Forbidden.Sentinel.CamLock.Normal.Radius
...
local function GetTarget()
    if Forbidden.Sentinel.Silent.mode == "target" then
        return TargetPlayer  
    elseif Forbidden.Sentinel.Silent.mode == "normal" then
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == Forbidden.Sentinel.Target.Keybind:lower() then
        if Forbidden.Sentinel.CamLock.Normal.Enabled then
            if IsTargeting then
    ...
    elseif Key:lower() == Forbidden.Sentinel.Target.UntargetKeybind:lower() then
        IsTargeting = false
        TargetPlayer = nil  
    end
end)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if Forbidden.Sentinel.CamLock.Normal.ClosestPart then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(Forbidden.Sentinel.CamLock.Normal.HitPart)
        end

        if BodyPart then
            local predictedPosition
            if Forbidden.Sentinel.CamLock.Normal.Aligner then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * Forbidden.Sentinel.CamLock.Normal.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * Forbidden.Sentinel.CamLock.Normal.Prediction)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if Forbidden.Sentinel.CamLock.Normal.SmoothnessEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, Forbidden.Sentinel.CamLock.Normal.Smoothness)
                else
                    Camera.CFrame = DesiredCFrame
                end
            end

            if Forbidden.Sentinel.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if Forbidden.Sentinel.Silent.mode == "target" then
                    local closestPoint
                    if Forbidden.Sentinel.Silent.TargetMode == "OptimalTargetPoint" then
                        closestPoint = GetOptimalTargetPoint(TargetPlayer.Character)
                    elseif Forbidden.Sentinel.Silent.TargetMode == "Closest Point" then
                        closestPoint = GetClosestPoint(TargetPlayer.Character)
                    elseif Forbidden.Sentinel.Silent.TargetMode == "BasicTargeting" then
                        closestPoint = GetClosestHitPoint(TargetPlayer.Character)
                    end

                    local velocity = GetVelocity(TargetPlayer, "Head")  
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * Forbidden.Sentinel.Silent.Prediction)
                elseif Forbidden.Sentinel.Silent.mode == "normal" then
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= Forbidden.Sentinel.Ranges.CloseDistance then
        return Forbidden.Sentinel.Silent.BoudingSizes.CloseFovRadius
    elseif distance <= Forbidden.Sentinel.Ranges.MidDistance then
        return Forbidden.Sentinel.Silent.BoudingSizes.MidFovRadius
    elseif distance <= Forbidden.Sentinel.Ranges.FarDistance then
        return Forbidden.Sentinel.Silent.BoudingSizes.FarFovRadius
    else
        return Forbidden.Sentinel.Silent.BoudingSizes.FarFovRadius  
    end
end
...
local function updateFov(TargetPlayer)
    local settings = Forbidden.Sentinel.Silent.BoudingSizes  
    clearFovParts()  
...
local function isPartInFovAndVisible(part)
    if not Forbidden.Sentinel.CamLock.Normal.Enabled or not IsTargeting or not TargetPlayer then
        return false
    end
...
local function isPartVisible(part)
    if not Forbidden.Sentinel.TargetControl.Safety then 
        return true  
    end
...
local OldPredictionY = Forbidden.Sentinel.Silent.Prediction
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and Forbidden.Sentinel.TargetControl.Aligner then
            Forbidden.Sentinel.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and Forbidden.Sentinel.TargetControl.Aligner then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            Forbidden.Sentinel.Silent.Prediction = OldPredictionY
            return velocity
        end
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and Forbidden.Sentinel.Silent.Aligner then
            Forbidden.Sentinel.Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and Forbidden.Sentinel.Silent.Aligner then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            Forbidden.Sentinel.Silent.Prediction = OldPredictionY
            return velocity
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[Forbidden.Sentinel.Target.Keybind:upper()] and CamLock.Normal.mode == "hold" then
        holdingMouseButton = false
    end

    if input.KeyCode == Enum.KeyCode[Forbidden.Sentinel.Target.UntargetKeybind:upper()] then
        IsTargeting = false  
        TargetPlayer = nil  
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if Forbidden.Sentinel.Silent.Enabled and IsTargeting then  
        UpdateFOV()  

        if Forbidden.Sentinel.Silent.mode == "target" and TargetPlayer then
...
task.spawn(function()
    while task.wait(0.1) do
        if Forbidden.Sentinel.Silent.Enabled then
            Fov.Visible = IsTargeting and Forbidden.Sentinel.Silent.AimSettings.FovSettings.FovVisible  
        end
    end
end)
...
if Forbidden.Sentinel.Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if Forbidden.Sentinel.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(Forbidden.Sentinel.Silent and Forbidden.Sentinel.Silent)
                if TargetVelv5 then
